...
export default function SharePage() {
  ...
  useEffect(() => {
    ...
    fetchReport(routeUid)
      ...
      .catch((fetchError) => {
        if (!mounted) return;
        setError(fetchError instanceof Error ? fetchError.message : "\u5206\u4eab\u5185\u5bb9\u52a0\u8f7d\u672a\u5b8c\u6210");
      })
      .finally(() => {
  ...
  const shareInfo = useMemo(() => {
    ...
    return {
      title: `${getReportTitle(report.reportType)} \u00b7 ${report.relationshipType || "\u5185\u5bb9\u6d1e\u5bdf"}`,
    ...
    return (
      <View className="page-shell">
        <View className="card report-state-card">
          <Text className="section-title">{"\u5206\u4eab\u5185\u5bb9\u52a0\u8f7d\u4e2d"}</Text>
  ...
  if (error || !report) {
    return (
      <View className="page-shell">
        <View className="card report-state-card">
          <Text className="section-title">{"\u5206\u4eab\u5185\u5bb9\u6682\u65f6\u4e0d\u53ef\u7528"}</Text>
          <Text className="section-subtitle">{error || "\u5185\u5bb9\u6570\u636e\u4e3a\u7a7a"}</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View className="page-shell share-page">
      ...
      <View className="card share-card">
        <Text className="section-title">{"\u5206\u4eab\u8fd9\u4efd\u5185\u5bb9"}</Text>
